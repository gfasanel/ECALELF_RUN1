...
plotPars={}
plotPars['invMass']=dict( name='inMass', nBins=100, xMin=40., xMax=200., xaxisLabel="M_{ee} (GeV)")
plotPars['EoverP']=dict( name='EoverP', nBins=100, xMin=0., xMax=2., xaxisLabel="E/p")
#plotPars['etaEleLead']=dict( name='etaEle[0]', nBins=100, xMin=-3., xMax=+3., xaxisLabel="#eta_{Lead Electron}")
...
Presel='((eleID[0] & 2)==2) && ((eleID[1] & 2)==2) && (HLTfire==1) && (recoFlagsEle[0] > 1) && (recoFlagsEle[1] > 1)'
...
for low in low_list:
    up=low+step
    pt_list.append('PtEle[0]>'+str(low)+'&& PtEle[0]<'+str(up))
    pt_label.append('Pt'+str(low)+str(up))
...
i=0
while i < len(pt_list):
    cuts['Presel_EB_gain0_'+pt_label[i]]=Presel+'&&'+EB+'&&'+gain0+'&&'+pt_list[i]
    cuts['Presel_EB_gain1_'+pt_label[i]]=Presel+'&&'+EB+'&&'+gain1+'&&'+pt_list[i]
    cuts['Presel_EB_gain2_'+pt_label[i]]=Presel+'&&'+EB+'&&'+gain2+'&&'+pt_list[i]
    cuts['Presel_EE_gain0_'+pt_label[i]]=Presel+'&&'+EE+'&&'+gain0+'&&'+pt_list[i]
    cuts['Presel_EE_gain1_'+pt_label[i]]=Presel+'&&'+EE+'&&'+gain1+'&&'+pt_list[i]
    cuts['Presel_EE_gain2_'+pt_label[i]]=Presel+'&&'+EE+'&&'+gain2+'&&'+pt_list[i]        
    i+=1
...
draw_opts['mc_Powheg']=dict( SetLineColor=kMagenta,SetLineWidth=2,SetFillColor=kMagenta,SetFillStyle=1001 )
